  ...
  Object.keys(this.propsData.data).forEach(function (key) {
    ...
    if (key in _plugin) {
      if (key === 'bezier') {
        ...
        style.transform = tweenStyle.transform ? _extends({}, tweenStyle.transform) : style.transform || getTransform(startData);
      }
      ...
      transform = tweenStyle.transform ? _extends({}, tweenStyle.transform) : style.transform || getTransform(startData);
      if (endUnit && endUnit.match(/%|vw|vh|em|rem/i)) {
        transform[key] = startConvertToEndUnit(_this2.target, computedStyle, key, transform[key], null, endUnit);
      }
